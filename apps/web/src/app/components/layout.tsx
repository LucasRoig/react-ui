...
function NavBlock({ title, children }: Readonly<{
  title: string;
...
function NavLink({ href, children }: Readonly<{
  href: string;
  children: React.ReactNode;
}>) {
  return <Link className="pl-5 text-sm text-muted-foreground-dark border-l border-transparent hover:text-dark-foreground hover:border-black -mx-px" href={href}>{children}</Link>
}
